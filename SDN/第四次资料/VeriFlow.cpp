...
bool VeriFlow::traverseForwardingGraph(const EquivalenceClass& packetClass, ForwardingGraph* graph, const string& currentLocation, const string& lastHop, vector< string > visited, FILE* fp)
{
	...
	for(int i = 0; i < visited.size(); i++){
		if(currentLocation == visited.at(i)){
			existed = true;
			break;
		}
	}
	if(existed)
	{
		...
		fprintf(stdout, "[VeriFlow::traverseForwardingGraph] Loop path is:\n");

		fprintf(fp, "%s", visited.at(0).c_str());
		cout << visited.at(0);
		for(int i = 1; i < visited.size(); i++){
			fprintf(fp, " -> %s", visited.at(i).c_str());
			cout << " <- " << visited.at(i);
		}
		fprintf(fp, "\n");
		cout << endl;
