...
class dynamic_rules(app_manager.RyuApp):
 ...
 # these two functions need to be coded in your own way
    def delete_flow(self, datapath, priority, match, actions, dst_port, idle_timeout=0, hard_timeout=0):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
    ...
    def get_OFPPortStatus_msg(self, ev):
        ...
        if reason == ofproto.OFPPR_MODIFY:
            if self.path:
                print("link has changed")
                for i in range(1, len(self.path) - 2, 1):
                    self.delete_flow(datapath=datapath, match=match, actions=actions, priority=100, dst_port=self.path[i + 1][1])
